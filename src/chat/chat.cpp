...
void Chat::setDeletedFromChat(const std::shared_ptr<User> &user,
  ...
  auto it = std::find_if(participants.begin(), participants.end(),
                         [&user](const Participant &participant) {
...
std::size_t
Chat::getLastReadMessageIndex(const std::shared_ptr<User> &user) const {
...
void Chat::updateLastReadMessageIndex(const std::shared_ptr<User> &user,
